...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/08/05 11:19:13 by tkhechoy          #+#    #+#             */
/*   Updated: 2023/08/05 11:37:37 by tkhechoy         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	mouse_fix(int button, int x, int y, t_window *window)
{
	...
	hit = closest_object(ray, window->scene, &window->object);
	free(window->type);
	window->type = NULL;
	window->type = ft_strdup(hit->type);
	free(hit->type);
	free(hit);
	return (0);
}
